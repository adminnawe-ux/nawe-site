    ...
    Deno.serve(async (req) => {
      ...
      // 3. Find matching session by payment_reference — covers both STK push (pending_stk)
      //    and manual NCBA payment (pending_verification) flows
      ...
      if (!session) {
        ...
        if (insertNotifErr) console.error('Failed to store unmatched notification:', insertNotifErr);
