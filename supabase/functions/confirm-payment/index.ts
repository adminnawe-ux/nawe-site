...
async function createCalendarEvent(params: {
  ...
  // Both were silently failing every time (caught by the try/catch around
  // createCalendarEvent below) since this function was written — meetLink/
  // calendarLink were always null. Neither is required for the core feature:
  // sessions.session_link already has a manual editor (SessionLinkEditor, used by
  // TherapistCalendar) the therapist can paste a Meet/Zoom link into, and that's
  // what ClientDashboard.tsx and TherapistCalendar.tsx already surface as "Join
  // session". This function's job is just to put the confirmed session on the
  // shared "Nawe Sessions" calendar.
