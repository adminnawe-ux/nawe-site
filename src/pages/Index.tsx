...
const regions = [
  { name: 'East Africa (HQ)',    countries: 'Kenya, Uganda, Tanzania',     status: 'Active',       statusClass: 'bg-primary/15 text-primary' },
  { name: 'West Africa',         countries: 'Nigeria, Ghana',              status: 'Expanding',    statusClass: 'bg-accent/15 text-accent' },
  { name: 'Southern Africa',     countries: 'South Africa, Zimbabwe',      status: 'Pilot Phase',  statusClass: 'bg-muted text-muted-foreground' },
  { name: 'Online / Diaspora',   countries: 'Global reach via platform',   status: 'Active',       statusClass: 'bg-primary/15 text-primary' },
];
...
const Index = () => (
  <div>
    ...
    <section className="relative min-h-screen flex items-center overflow-hidden bg-primary">
      {/* Decorative blobs using existing primary shades */}
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-3xl animate-fade-in">
          <p className="font-ui text-primary-foreground/70 text-sm uppercase tracking-widest mb-6">
            Equitable Access to Mental Health Care Across Africa
          </p>
          <h1 className="font-display text-5xl md:text-7xl text-primary-foreground leading-tight mb-4">
            Find Your Calm.
          </h1>
          <h1 className="font-display text-5xl md:text-7xl text-accent leading-tight mb-8">
            Find Your Person.
          </h1>
          <p className="font-body text-lg md:text-xl text-primary-foreground/80 mb-10 max-w-xl leading-relaxed">
            Connect with licensed, culturally-competent therapists who understand you — wherever you are in Africa.
          </p>
          <div className="flex flex-col sm:flex-row gap-4">
            <Link to="/questionnaire">
              <Button size="lg" className="font-ui text-base px-8 py-6 rounded-full bg-primary-foreground text-primary hover:bg-primary-foreground/90 shadow-soft">
    ...
    <section className="py-24 bg-card border-y border-border">
      <div className="container mx-auto px-6">
        ...
        <div className="bg-primary rounded-card p-8 mb-10">
          <p className="font-body text-lg text-primary-foreground italic text-center leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { label: 'Who We Are',  color: 'bg-primary text-primary-foreground',   body: 'Nawe is a Kenyan-based social enterprise founded to address the critical gap in mental health care access across Sub-Saharan Africa. We combine technology, public health expertise, and a deep commitment to equity.' },
            { label: 'The Problem', color: 'bg-accent text-accent-foreground',      body: 'Over 75% of people with mental health conditions in Africa receive no treatment. High costs, limited providers, stigma, and geographic barriers leave millions without support.' },
            { label: 'Our Solution',color: 'bg-primary text-primary-foreground',   body: 'A digital matching platform that connects clients to therapists of their choice with standardized pricing, flexible scheduling, and multilingual support — bridging the access gap at scale.' },
          ].map((c) => (
            <div key={c.label} className="bg-background border border-border rounded-card overflow-hidden shadow-card">
              <div className={`${c.color} px-5 py-3 font-ui text-sm font-medium`}>{c.label}</div>
    ...
    <section className="py-24 bg-foreground">
      <div className="container mx-auto px-6">
        <div className="inline-block border border-background/20 rounded-full px-4 py-1 font-ui text-xs text-background/60 uppercase tracking-widest mb-8">How It Works</div>
        <h2 className="font-display text-4xl md:text-5xl text-background mb-16">Your Journey to Better Mental Health</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {steps.map((s, i) => (
            <div key={s.n} className="relative">
              {i < steps.length - 1 && (
                <div className="hidden lg:block absolute top-6 left-full w-full h-px bg-background/20 -translate-y-1/2 z-0" />
              )}
              <div className="relative z-10">
                <div className="w-12 h-12 rounded-full bg-primary flex items-center justify-center mb-4">
                  <span className="font-ui text-sm font-bold text-primary-foreground">{s.n}</span>
                </div>
                <p className="font-display text-accent mb-2">{s.title}</p>
                <p className="font-body text-sm text-background/70 leading-relaxed">{s.body}</p>
              </div>
            </div>
          ))}
        </div>
    ...
    <section className="py-24 bg-card border-y border-border">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {regions.map((r) => (
            <div key={r.name} className="bg-background border border-border rounded-card p-6 shadow-card">
    ...
    <section className="py-24 bg-background">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {partners.map((p) => (
            <div key={p.title} className="bg-card border border-border rounded-card p-6 shadow-card">
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-4">
            {resources.map((r) => (
              <div key={r.n} className="flex gap-5 bg-background border border-border rounded-card p-5 shadow-card">
                <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center shrink-0">
                  <span className="font-ui text-sm font-bold text-primary-foreground">{r.n}</span>
                </div>
                <div>
          ...
          <div className="bg-primary rounded-card p-8 flex flex-col justify-between">
            <div>
              <h3 className="font-display text-2xl text-primary-foreground mb-4">Stay Informed</h3>
              <p className="font-body text-sm text-primary-foreground/80 leading-relaxed mb-6">
            ...
            <div className="space-y-3">
              <input
                ...
                className="w-full rounded-full px-4 py-3 font-ui text-sm bg-primary-foreground/10 border border-primary-foreground/20 text-primary-foreground placeholder:text-primary-foreground/50 outline-none focus:border-primary-foreground/60"
              />
              <a href={`mailto:${SUPPORT_EMAIL}`}>
                <Button className="w-full font-ui rounded-full bg-accent hover:bg-accent/90 text-accent-foreground">
                  Subscribe Now <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              </a>
              <p className="font-ui text-xs text-primary-foreground/60 text-center">{SUPPORT_EMAIL}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section className="py-24 bg-background">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {testimonials.map((t) => (
            <div key={t.name} className="bg-card rounded-card p-8 shadow-card border border-border">
